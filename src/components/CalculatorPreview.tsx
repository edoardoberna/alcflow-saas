...
export default function CalculatorPreview({
  ...
  // Valori dinamici degli slider e campi numerici
  ...
  // Sincronizza lo stato ogni volta che cambiano gli input dal builder
  useEffect(() => {
    setValues((prev) => {
      const updated = { ...prev };
      inputs.forEach((inp) => {
        if (inp.variable) {
          if (updated[inp.variable] === undefined || isNaN(updated[inp.variable])) {
  ...
  // Se l'opzione enableLeadGate viene disattivata dal Builder, sblocca i risultati immediatamente
  useEffect(() => {
    if (!enableLeadGate) {
      setIsUnlocked(true);
    }
  ...
  // Auto-resize via postMessage per iframe
  useEffect(() => {
    const notifyHeight = () => {
  ...
  // Ricalcolo immediato dei risultati
  const calculatedOutputs = useMemo(() => {
    return outputs.map((out) => {
      const numericResult = evaluateFormula(out.formula, values);
      return {
        ...out,
    ...
    try {
      ...
      if (webhookUrl && webhookUrl.startsWith('http')) {
        try {
          ...
          // Ignore third-party network issues
        }
      }
      ...
      setSubmitError(err.message || 'Errore durante la trasmissione del contatto.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div
      ref={containerRef}
      className="w-full max-w-2xl mx-auto rounded-2xl border border-white/10 bg-[#080C14] text-white p-6 sm:p-8 shadow-2xl transition-all"
        ...
        {enableLeadGate && !isUnlocked ? (
          <div className="p-6 rounded-xl border border-white/10 bg-[#0C1019] space-y-4">
            ...
            <form onSubmit={handleLeadSubmit} className="space-y-3.5">
              ...
              {/* Checkbox Privacy Obbligatorio e Cliccabile */}
